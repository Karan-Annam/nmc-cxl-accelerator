...
`ifdef HDM_WORDS
  // FPGA capacity config: the xc7s50 (Urbana board) holds at most 32K words
  // of dual-read-port HDM entirely in block RAM (64 of 75 RAMB36; the full
  // 64K map needs 128 and spills half the banks into timing-breaking async
  // LUTRAM). Simulation always runs the full 64K map; synth.tcl passes
  // HDM_WORDS=32768.
  parameter int SRAM_DEPTH = `HDM_WORDS;
`else
...
`endif
  ...
  parameter int BANK_AW    = $clog2(BANK_DEPTH);    // 13 (12 in the FPGA config)
